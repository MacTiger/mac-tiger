		...
		for (Map.Entry<String, FunctionOrVariable> entry: this.currentTDS.getParent().getFunctionsAndVariables()) {
			...
			switch (name) {
				case "concat": {
					...
					this.writer.writeHeader(String.format("STW %s, (HP)+", size3reg)); //TODO : corriger ce qui était +(HP) : ce n'est pas accepté par l'assembleur !
					...
					this.writer.writeHeader(String.format("ANI %s, %s , #0xFF00", charReg, charReg));
					this.writer.writeHeader(String.format("BMP 4"));
					this.writer.writeHeader(String.format("ANI %s, %s, #0x00FF", charReg, charReg));
