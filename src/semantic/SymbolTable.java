		...
		switch (tree.getType ()) {
			...
			case SEQ:
				for (int i = 0; i < tree.getChildCount(); i++) {
					expType = this.fillWith(tree.getChild(i));
				}
				return expType;
			...
			case CALL:
				...
				for (int i = 0; i < expectedArgs.size(); i++) {
					this.checkType(tree.getChild(i + 1), expectedArgs.get(i).getType(), false);
				}
			...
			// case ID:
			case STR:
				return this.fillWithSTR(tree);
			case INT:
				return this.fillWithINT(tree);
		}
